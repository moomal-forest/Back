...
router.get('/:userId/neighbors', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: '이웃 목록을 가져오는데 실패했습니다. 다시 시도해 주세요.' });
  }
});
...
router.post('/:userId/neighbors', async (req, res) => {
    try {
      ...
      if (!neighborToAdd) {
        return res.status(404).json({ error: '추가할 사용자를 찾을 수 없습니다.' });
      }
      
      // 자기 자신을 이웃으로 추가하려는 경우 에러
      if (user._id.toString() === neighborToAdd._id.toString()) {
        return res.status(400).json({ error: '자기 자신은 이웃으로 추가할 수 없습니다.' });
      }
...
router.delete('/:userId/neighbors/:neighborId', async (req, res) => {
  try {
      ...
      // 현재 사용자에서 이웃 제거 (양방향 삭제)
      if (!user.neighbors.includes(neighborToRemove._id)) {
          return res.status(400).json({ error: '이웃 목록에 없습니다.' });
      }

      user.neighbors = user.neighbors.filter(neighbor => !neighbor.equals(neighborToRemove._id));
      await user.save();

      // 이웃 사용자에서 현재 사용자 제거 (양방향 삭제)
      if (neighborToRemove.neighbors.includes(user._id)) {
          neighborToRemove.neighbors = neighborToRemove.neighbors.filter(neighbor => !neighbor.equals(user._id));
